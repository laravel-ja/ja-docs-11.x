...
Reverbはサーバ上のWebSocket接続を管理するために、ReactPHPのイベントループを使用しています。このイベントループはデフォルトで、`stream_select`によって動かされ、追加の拡張は必要ありません。しかし、`stream_select` は通常、１，０２４個のファイルオープンに制限されています。そのため、１，０００を超える同時接続を処理する場合は、同じ制限に限定されない、別のイベントループを使用する必要があります。
